...
export async function createUser(email, password, username) {
  ...
  export const getBookmarkPosts = async (userId) => {
    try {
      const posts = await databases.listDocuments(
        appwriteConfig.databaseId,
        appwriteConfig.videoCollectionId,
        [Query.contains('savedbyusers', userId)]
    )
    return posts.documents;
    } catch (error) {
      console.log(error);
  ...
  export async function savePost(postId, userId) {
    try {
      ...
      const existingSavedByUsers = postDocument.savedby;
      
      const updatedLikeByUsers = [...existingSavedByUsers, userId];
  ...
  export const uploadFile = async (file, type) => {
    ...
    try {
      ...
      console.log(type, 'uploaded File',uploadedFile);
